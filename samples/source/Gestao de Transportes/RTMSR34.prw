...
If ( lMDFEAUT .And. IsInCallStack("TmsMDFeAut") .And. Len(PARAMIXB[1]) == 8 ) .Or. ( (IsInCallStack("TMSAI86") .Or. IsInCallStack("TMSAC30Man")) .And. Len(PARAMIXB[1]) == 10 )

	Pergunte(cPerg,.F.)	

	nCntFor  := Len(PARAMIXB)	
	mv_par01 := PARAMIXB[nCntFor][8] //FILORI VIAGEM
	mv_par02 := PARAMIXB[nCntFor][5] //VIAGEM DE
	mv_par03 := PARAMIXB[nCntFor][5] //VIAGEM ATE
	mv_par04 := PARAMIXB[nCntFor][1] //FILMAN DE
	mv_par05 := PARAMIXB[nCntFor][1] //FILMAN ATE
	mv_par06 := PARAMIXB[1][2]       //MANIFESTO DE
	mv_par07 := PARAMIXB[nCntFor][3] //MANIFESTO ATE
	mv_par08 := PARAMIXB[nCntFor][4] //SERIE MAN DE
	mv_par09 := PARAMIXB[nCntFor][4] //SERIE MAN ATÉ
	lImp     := PARAMIXB[nCntFor][7] //Tela de Setup (.T. não exibe)
	If Len(PARAMIXB[1]) == 10
		cFilePrint := PARAMIXB[nCntFor][09] //Nome do arquivo PDF
		cStartPath := PARAMIXB[nCntFor][10] //Pasta para gerar o arquivo
	EndIf
...
oDamdfe:= FWMSPrinter():New(cFilePrint, IMP_PDF, .F., cStartPath, lImp, , @oDamdfe, ,Iif(":" $ cStartPath,.F.,.T.) , , , !lImp )
