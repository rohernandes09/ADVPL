...
Local nTotCTRC :=0
Local nPesoTot :=0
Local nValFre:=nValAdi:=nValPdg:=nIRRF:=nSEST:=nINSS:=nTotValFre:=0
...
Do While !Eof() .And. DTY->DTY_FILIAL == xFilial("DTY") .And. DTY->DTY_FILORI <= mv_par02 .And.;
	...
	nTotCTRC:=nPesoTot:=0
	nTotCapac:=nValFre:=nValAdi:=nValPdg:=nIRRF:=nSEST:=nINSS:=nTotValFre:=0
	
	DTQ->( dbSetOrder(1) )
	If !DTQ->(MsSeek(xFilial("DTQ") + DTY->DTY_VIAGEM))
	   DTQ->(dbSkip())
		Loop
	EndIf
	...
	@Li,045 PSay "INSS....................: " + TransForm(nINSS,PesqPict("DTY","DTY_INSS"))
	VerLin(@Li,1)	
	@Li,000 PSay STR0010 + TransForm(nTotCapac,PesqPict("DA3","DA3_CAPACM")) //"Peso Pago...............: "
	nSldPag := nTotValFre - (nValAdi+nIRRF+nSEST+nINSS)
