...
Private cDiscrNFSe := ""
Private cField     := "" 
Private cMVBENEFRJ := AllTrim(GetNewPar("MV_BENEFRJ"," ")) 
Private cF4Agreg	 := ""
Private cFieldMsg  := ""
Private cTpPessoa	 := ""
Private cUFxIss	 := ""
...
static function valoresNac( aISSQN, aRetido, aTotal, aDest, cCodMun,aLeiTrp,lRecIrrf, aProd )
    ...
    cString += IIF( SF4->F4_DESCOND == '1' .and. !Empty( nDescon ), '<vDescCond>' + convType(nDescon,15,2) + '</vDescCond>', '<vDescCond>0.00</vDescCond>')
    cString += '</vDescCondIncond>'
    ...
    cString += IIF(!empty( aISSRet[2] ),'<tpRetPisCofins>2</tpRetPisCofins>','<tpRetPisCofins>1</tpRetPisCofins>')
    cString += '</piscofins>'
