...
±±³            ³        ³      ³da a limpeza.                             ³±±
±±³Cícero Alves³04/11/16|TWKETY³Ajuste para imprimir a descrição do curso ³±±
±±³            ³        |	   ³corretamente							  ³±±
...
If nPos+nLi < 2790 .And. nLi < 2770
	oPrint:line(nPos,035 ,nPos,2350) 		//Linha Horizontal
	nPos+=05
	oPrint:say (nPos,040 ,STR0073,cFont)	//CARACTERÍSTICAS
	nPos+=40
	oPrint:line(nPos,035 ,nPos,2350) 		//Linha Horizontal
	nPos+=35
Else
...
While SM6->(!Eof() .And. SM6->M6_FILIAL + SM6->M6_CURRIC == xFilial("SM6")+SQG->QG_CURRIC)
	...
	Else
		...
		oPrint:say(nPos,040 ,SPACE(05)+Padr(RS6->RS6_DESC,30)+Space(20)+STR0074,cFont)//"Características Selecionadas"
		nPos+=50
		For nX := 1 to Len(aAux)
