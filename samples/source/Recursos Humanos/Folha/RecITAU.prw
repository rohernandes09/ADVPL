...
Static Function GERRImp1()
	...
	*/
	...
	ElseIf cTipoRot == '2'		// Adiantamento
		cFinPgt := "02"
	ElseIf cTipoRot == '5' .or. cTipoRot == '6'	// 1a ou 2a Parcela 13°
		cFinPgt := "04"
	ElseIf cTipoRot == 'F'		// Participação de lucro
		cFinPgt := "05"		
	Else			           // Extras
		cFinPgt := "10"
	EndIf
	...
	If SRA->RA_SITFOLH == "F"
		If SRF->(dbSeek( SRA->(RA_FILIAL+RA_MAT) ))
			If SRH->(dbSeek( SRA->(RA_FILIAL+RA_MAT)+Dtos(SRF->RF_DATABAS) ))
				While SRH->(!EoF()) .And. (SRH->RH_FILIAL+SRH->RH_MAT == SRA->RA_FILIAL+SRA->RA_MAT) .And. (Dtos(SRH->RH_DATABAS) >= Dtos(SRF->RF_DATABAS))
