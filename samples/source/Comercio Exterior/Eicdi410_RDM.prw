...
IndRegua('Work',cNomArq+TEOrdBagExt(),'JD_TIPO_CT+DTOS(JD_DT_ENT)')

cNomArq2:=CriaTrab(Nil,.f.)
IndRegua('Work',cNomArq2+TEOrdBagExt(),'JD_ARMADOR+JD_TIPO_CT')

cNomArq3:=CriaTrab(Nil,.f.)
IndRegua('Work',cNomArq3+TEOrdBagExt(),'JD_CONTAIN')

Set Index to (cNomArq+TEOrdBagExt()),(cNomArq2+TEOrdBagExt()),(cNomArq3+TEOrdBagExt())
...
Private aRotina  := { { STR0001   ,'AxPesqui'   , 0 , 1},; //"Pesquisar" //LGS-27/04/2015
                    { STR0002   ,'Eval(bEdit)', 0 , 4},; //"Manut. Cont's"
                    { STR0003   ,'Eval(bEsto)', 0 , 2},;//"Estorno Cont's"
                    { "Pequisar Container", 'U_DI410PesqCont()',0,1}} //Pequisar Container
Private aRotAdic := {}  // GFP - 13/03/2014
...
While .T.
  ...
  TB_Campos:= AddCpoUser(TB_Campos,"SJD","5","Work")
   ...
   aRotina := MenuDef()   // GFP - 02/05/2012 - Carrega aRotina novamente para que os campos sejam alteraveis quando inclus„o e/ou alteraÁ„o.
...
IF Work->(BOF()) .AND. Work->(EOF()) .AND. (nManut==ALTERAR .OR. nManut==EXCLUIR)
   ...
   RETURN .F.
ENDIF
IF Empty(cArmador)  // GFP - 14/10/2013
   MsgInfo(STR0068,STR0067)  // "O Armador deve ser informado." ### "Aviso"
   RETURN .F.
ENDIF
bOk    :={||DI410EndValid()}
...
If AVFlags("DEMURRAGE") // FSM - 22/03/11 - Novo rotina para controle de Demurrage/Detention -
   ...
   aChange:={'JD_HAWB'   ,'JD_ARMADOR','JD_DT_ENT' ,'JD_CONTAIN','JD_TIPO_CT', 'JD_QTD_REC','JD_DTPREVI' ,'JD_DEVOLUC'}  // GFP - 16/10/2013 //LRS - 19/09/2014 - Habilitar o Campo JD_DT_ENT
...
Else
   ...
   aChange:={'JD_HAWB'   ,'JD_ARMADOR','JD_DT_ENT' ,'JD_CONTAIN','JD_TIPO_CT',;
             ...
             'JD_ATRASO' ,'JD_TOTALPG','JD_PAGO_EM','JD_DEPANTE','JD_TOTALGE'}  // GFP - 16/10/2013
EndIf
...
M->JD_DT_ENT := If(nManut == INCLUIR,SW6->W6_CHEG,Work->JD_DT_ENT)  // GFP - 16/10/2013
...
IF nManut == INCLUIR
    ...
    While SX3->(!Eof()) .And. SX3->X3_ARQUIVO == "SJD"
       If SX3->X3_PROPRI=="U" .AND. X3Uso(SX3->X3_USADO)
          If SX3->X3_TIPO == "C"
             M->&(SX3->X3_CAMPO):= Space(SX3->X3_TAMANHO) //MCF-14/01/2015
...
/* //*** GFP - 18/08/2011 :: 17h26 - Nopado pois campo È criado em ambos os casos de verificaÁ„o do AVFLAGS
...
IF nManut #EXCLUIR
   ...
   FOR I := 1 TO LEN(ATabValid)
      ...
      ENDIF
   NEXT
   // FDR - 04/11/10 - ValidaÁ„o do n˙mero do container
   If nManut == INCLUIR
      Work->(DbSetOrder(3))
      If Work->(DbSeek(M->JD_CONTAIN))
         MsgInfo("N˙mero de container j· cadastrado para este processo.")
         RETURN .F.
      ENDIF
   ENDIF
ENDIF
...
IF nManut #EXCLUIR
   ...
   FOR Ind := 1 TO (Work->(FCOUNT())-2)// -2 para nao pegar o campo DELETE e WKRECNO da Work
       If !(Work->(FieldName(Ind)) $ "DBDELETE/WKRECNO/TRB_ALI_WT/TRB_REC_WT")
...
nLin :=5; nColS:=05; nColG:=40; nAltu:=7; nPula:=12
 ...
 @ nLin,nColG msGET SW6->W6_DT_DESE    WHEN .F. SIZE 33,nAltu of oPanel Pixel
   nLin:=nLin+nPula
 @ nLin,nColG msGET cArmador F3 "EIA" VALID fArmNome(cArmador) WHEN fArmEmpty(cArmador) SIZE 30,nAltu of oPanel Pixel  // GFP - 14/10/2013
   nLin:=nLin+nPula
...
If !Empty(M->&(cNomeCampo))
	...
	If Upper(cNomeCampo) == 'JD_DEVOLUC'
       // FDR - 30/10/10 - Verifica se a Data de AtracaÁ„o no DesembaraÁo est· preenchida
       If Empty(SW6->W6_CHEG)
          cMsg += "Preencher a Data de AtracaÁ„o no DesembaraÁo." + ENTER
          lRet := .F.
	...
	ElseIf M->&(cNomeCampo) < M->JD_DT_ENT
	       cMsg += "A data " + If(Upper(cNomeCampo) == 'JD_DTPREVI',"prevista ","") + "de devoluÁ„o deve ser maior que a data de entrada." + ENTER
           lRet := .F.
	EndIf
...
If !lRet
   MsgInfo("Para o correto preenchimento do campo:" + Replic(ENTER,2) +;
           cMsg,"AtenÁ„o")
EndIf
...
Begin sequence
	  If AVFlags("DEMURRAGE")
	  		aItens := {"Numero de Container"}
	  EndIf
     DEFINE MSDIALOG oDlgCont FROM 0,0 TO 10,80  TITLE cTit OF oMAINWND //FDR - 30/08/12 - Ajuste na tela

        oPanel:=TPanel():New(0, 0, "", oDlgCont,, .F., .F.,,, 0, 0)
        oPanel:Align:= CONTROL_ALIGN_ALLCLIENT
		  If AVFlags("DEMURRAGE")
		  		@ 15,027 Radio nRadio  ITEMS aItens[1]  size 100,10 of oPanel pixel
		  Else
        		@ 15,027 Radio nRadio  ITEMS aItens[1],aItens[2]  size 100,10 of oPanel pixel
		  EndIf
     ACTIVATE MSDIALOG oDlgCont ON INIT EnchoiceBar(oDlgCont,bOk,bCancel,,,,,,,.F.) CENTERED
     ...
     If nRadio == 1
        cTit := "Pesquisa por N∫ de Container"
        cSay := AVSX3("JD_CONTAIN",5)
        cPic := "@R XXXX-XXXXXX/X"//Acb - 24/10/2010
     EndIf

     If nRadio == 2
        cTit := "Pesquisa por N∫ de Cadeado"
        cSay := AVSX3("JD_CADEADO",5)
        cPic := "@!" // acb - 24/10/2010
     End If

     DEFINE MSDIALOG oDlgChave FROM 0,0 TO 10,80 TITLE cTit OF oMAINWND

        oPanel:=TPanel():New(0, 0, "", oDlgChave,, .F., .F.,,, 0, 0)
        oPanel:Align:= CONTROL_ALIGN_ALLCLIENT

        @ 14,025 SAY cSay OF oPanel PIXEL
        @ 21,025 MSGET oChave VAR cChave Picture cPic OF oPanel SIZE 100,10 PIXEL
...
//        DEFINE SBUTTON FROM 29,100 TYPE 2 OF oDlgChave ENABLE ACTION (oDlgChave:End())
     ...
     ACTIVATE MSDIALOG oDlgChave ON INIT EnchoiceBar(oDlgchave,bOk2,bCancel2,,,,,,,.F.) CENTERED
     ...
     If nOpcao == 1
        ...
        If nRadio == 1
           ...
           Else
              MsgInfo("N„o possui nenhum embarque com o n˙mero do container informado!","Aviso")
           EndIf
        ...
        SJD->(DbSetOrder(4))
           If SJD->(DbSeek(xFilial("SJD")+AvKey(cChave,"JD_CADEADO")))
              ...
              SW6->(DbSeek(xFilial("SW6")+SJD->JD_HAWB))
           Else
              MsgInfo("N„o possui nenhum embarque com o n˙mero de cadeado informado!","Aviso")
           EndIf
        EndIf
...
/*
Funcao      : fArmEmpty()
Parametros  : cArmador :: CÛdigo Armador
Retorno     : lRet :: .T./.F.
Objetivos   : ValidaÁ„o do When do campo Armador
Autor       : Guilherme Fernandes Pilan - GFP
Data/Hora   : 14/10/2013 :: 17:57
*/
*------------------------------------*
Static Function fArmEmpty(cArmador)
*------------------------------------*
Local lRet := .T.
